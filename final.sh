...
# Melakukan pergantian resolv.conf
echo [INFO] "Memulai pergantian resolv.conf"
sed -i '2{N;s/\(.*\)\n\(.*\)/\2\n\1/;}' /etc/resolv.conf
echo [INFO] "Nameserver 10.0.0.2 telah berada di atas"

# Melakukan pengunduhan file wordpress
echo [INFO] "Memulai pengunduhan file wordpress"
wget -P /var/www/html/ dl.ukk.my.id/latest.zip
...
# Melakukan pengembalian resolv.conf
echo [INFO] "Memulai pengembalian resolv.conf"
sed -i '2{N;s/\(.*\)\n\(.*\)/\2\n\1/;}' /etc/resolv.conf
echo [INFO] "Nameserver 172.16.18.2 telah berada di atas"
...
echo [INFO] "Harap rubah DNS di mikrotik dengan 172.16.18.2 sebagai DNS server."
